...
class EcsClient(object):
    def __init__(self, access_key_id=None, secret_access_key=None,
                 region=None, profile=None, config_file=None, environment=None):
        ...
        self.template_renderer = TemplateRenderer(self.config_file, self.environment)
    ...
    def register_task_definition(self, task_definition_file):
        task_definition = self.template_renderer.render(task_definition_file)

        return self.boto.register_task_definition(**task_definition)
    ...
    def create_service(self, service_file):
        service_params = self.template_renderer.render(service_file)
        return self.boto.create_service(**service_params)

    def update_service(self, service_file):
        service_params = self.template_renderer.render(service_file)
        if 'loadBalancers' in service_params:
            del service_params['loadBalancers']
        if 'launchType' in service_params:
            del service_params['launchType']
        if 'placementStrategy' in service_params:
            del service_params['placementStrategy']
        if 'placementConstraints' in service_params:
            del service_params['placementConstraints']
            
        service_name=service_params['serviceName']
        del service_params['serviceName']
        return self.boto.update_service(service=service_name, **service_params)
